...
class SirenHubAdd1 extends Component {
  ...
  handleOnNext(event){
  	this.props.history.push('/device/sirenHubAdd2');
  }
  ...
  render() {
    ...
    return (
      <div className="device addSiren">
        <BarTitle onBack={this.handleClickBack} title={Lang.device.sirenhubAdd1.title} />
        <ScrollView>
	        <div className="addWrap">	        	
	        	<p className="addDesc">{Lang.device.sirenhubAdd1.addDesc}</p>      	
	        	<div className="nextStep backgroud-Siren-Hub1"></div>	                    
	        </div>
	        <button className ="nextBtn" onClick={this.handleOnNext}>{Lang.device.sirenhubAdd1.nextStep}</button>	
        </ScrollView>
      </div>
    );
  }
