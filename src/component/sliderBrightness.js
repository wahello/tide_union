...
export default class extends Component {
  ...
  handleBeforeChange(value,event){
  	console.log("handleBeforeChange - ",value);
  }
  onLChange(value, event){
  	console.log("onLChange - ",value);
  ...
  handleChange(value, event){
  	console.log("handleChange - ",value);
		...
		if(!this.timerTouchMove){
	  		...
	  		that.props.onAfterChange && that.props.onAfterChange(value);
				that.setState({value});
		  	}, 200);
		}
		that.setState({value});
  	...
  	let styles = {
        ...
        padding: "0.66rem 0 0",
  		...
  		tip:{
  			...
  			display: "inline-block",
  			top: "calc(-5.29rem * .5 )",
  			left: 'calc(-7.33rem * .5 + ' + brightness + '%)',
    ...
    return (
    	<div style={styles.root}>
    		...
    		<div className="tip-wrap">
    			<span className="tip" style={styles.tip}>{brightness + "%"}</span>
    		</div>
			</div>
    );
  }
